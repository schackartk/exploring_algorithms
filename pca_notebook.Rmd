...
* Linear regression based on `power` ($r^2=0.611$)
* Linear regression based on `weight` ($r^2=0.691$)
* Multilinear regression based on both `weight` and `hp` ($r^2=0.784$)

Of course, this is only a single observation, so we can't be sure if the improvement is significant. Still, it is interesting.
...
reg_rs <- round(summary(rot_fit$fit)$adj.r.squared, digits = 3)
  
df_rot %>% ggplot(aes(x=x, y=mpg)) +
...
Apparently, we could do this same process again. And again..
...
It mostly comes down to how we performed the regression. As mentioned, we used
ordinary least-squares regression, which minimized the vertical errors. By 
...
uncorrelated as indicated by the nearly horizontal regression line
