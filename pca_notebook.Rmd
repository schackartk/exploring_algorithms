...
* We can build linear regression models based on each predictor
* A multilinear regression model based on all predictors seems to be a better fit
* Multilinear regression has more terms than regular linear regression
...
Looking at the last plot, we can see that if we use this technique, then when we project the points to the regression line, the variable with smaller magnitude doesn't really seem to matter. Effectively, our new axis based on the regression line is pretty much the same as just using the `power` or $y$ axis as our predictor.
...
```{r rot, echo=FALSE, message=FALSE}
train <- train %>% reg_coord(x="wt_norm", y="hp_norm") %>% 
  left_join(train) %>% 
...
* Each variable can be rescaled by dividing by its standard deviation
* Once each variable is rescaled, they have equal magnitude
* Each variable then contributes equally to the new axis
* This new single variable effectively captures the information of both original variables leading to similar predictive strength
...
grid.arrange(plt$mpg_reg1_fit + labs(x="reg1"), plt$mpg_ortho1, ncol=2)
```
...
* Ordinary least-square regression is inappropriate as a new axis for a few reasons:
  * Minimizes *vertical* errors
  * Dependent on which variable is on which axis
  * New variables are still correlated
* Orthogonal regression is advantageous:
...
* The directions given by the eigenvectors of the covariance matrix are equivalent to the orthogonal regression projection axes
* The eigenvalues associated with each eigenvector represent the amount of variance in that direction
* The directions given by the eigenvectors are referred to as principal axes
* PCA is sensitive the *re-centering*, which we will explore further
...
```{r, echo = false}
reg$dem_zero_int <- c(
  slope = mean(train$hp_norm) / mean(train$wt_norm),
  intercept = 0
)

train %>% ggplot(aes(x=wt_norm, y=hp_norm)) +
  geom_point() +
  geom_abline(slope=reg$dem_zero_int["slope"],
              intercept=reg$dem_zero_int["intercept"]) +
  geom_abline(slope=reg$dem["Slope"],
              intercept=reg$dem["Intercept"],
              color="gray") +
  ...
  labs(x="Rescaled Weight",
       ...
       subtitle="Gray: standard Black: through origin")

```
Now, we can project to this new regression line. I will call our new axes that result from this projection `ortho_no_1` and `ortho_no_2`.

Here is `ortho_no_1` plotted against `PC1`.

```{r, echo=FALSE}

ortho_no_segments <- perp_segment_coord(train$wt_norm, train$hp_norm,
                                    reg$dem_zero_int["intercept"],
                                    reg$dem_zero_int["slope"]) %>% 
  as.data.frame()

train <- train %>% add_column(
  ortho_no_1 = sqrt((ortho_no_segments$x1)^2 +
                  (ortho_no_segments$y1 - reg$dem_zero_int["intercept"])^2),
  ortho_no_2 = sqrt(
    (ortho_no_segments$x1 - ortho_no_segments$x0)^2 +
      (ortho_no_segments$y1 - ortho_no_segments$y0)^2))

train <- train %>% mutate(ortho_no_1 = case_when(
  ortho_no_segments$x1 < 0 ~ -ortho_no_1,
  ortho_no_segments$x1 >= 0 ~ ortho_no_1
)) %>% 
  mutate(ortho_no_2 = case_when(
    ortho_no_segments$y0 < ortho_no_segments$y1 ~ -ortho_no_2,
    ortho_no_segments$y0 >= ortho_no_segments$y1 ~ ortho_no_2
  ))

reg$PC1_ortho_no_1 <- lm_model %>% 
  fit(PC1 ~ ortho_no_1, data=train)
```

```{r, echo=FALSE}
train %>% ggplot(aes(x=ortho_no_1, y=PC1)) +
  geom_abline(slope=get_coef(reg$PC1_ortho_no_1, "ortho_no_1"),
              intercept=get_coef(reg$PC1_ortho_no_1),
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(title="PC1 vs. ortho_no_1",
       subtitle=str_glue("r square = {get_rsq(reg$PC1_ortho_no_1,3)}"))
```
...
To illustrate this, I can show what the first principal component (or orthogonal regression with fixed zero intercept) looks like on some fake data. As mentioned, the fit solely depends on the intercept and centroid, and ignore the actual slope of the data.

```{r}
df_fake$x_2 <- c(0.2, 0.27, 0.32, 0.39, 0.44, 0.44, 0.48,
                  0.51, 0.59, 0.68, 0.72, 0.83, 0.95, 1.0)

df_fake$y_2 <- c(0.97, 0.86, 0.86, 0.75, 0.7, 0.65, 0.49,
                 0.50, 0.41, 0.45, 0.37, 0.26, 0.30, 0.11)

plt$y_2_x_2 <- df_fake %>% 
  ggplot(aes(x=x_2, y=y_2)) +
  geom_abline(slope=mean(df_fake$y_2) / mean(df_fake$x_2),
              intercept=c(0,0),
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  ...
  labs(x="Variable 1",
       y="Variable 2",
       title="First Principal Component of Uncentered Data",
       subtitle="Equivalent to Orthogonal Regression, 0 Intercept")
...
With this in mind, we have to consider our constraints. As I stated, principal components (via matrix operations) always pass through the origin. So, we cannot get around that aspect when moving from orthogonal regression to principal component analysis.
...
* The standard deviation is 1, which for us means that each variable contributes equally to the regression for projection
* The mean is 0, which means that the orthogonal regression can be optimal while passing through the origin as is necessary for PCA.
...
max_scale <- max(abs(train$hp_z), abs(train$wt_z))

plt$hp_z_tw_z <- train %>% ggplot(aes(x=wt_z, y=hp_z)) +
  geom_point(color=get_hurwitz_colors("red")) +
  geom_point(x=mean(train$wt_z), y=mean(train$hp_z),
             color="darkgray",
             pch="+", size=5) +
  lims(x=c(-max_scale, max_scale),
       y=c(-max_scale, max_scale)) +
  labs(title="Standardized Variables",
       subtitle="Centroid marked with +",
       x="Standardized Weight",
       y="Standardized Power")
...
### Dimension Reduction of Standardized Variables {.tabset}

Using the standardized variables, we can compare (a) the projection to orthogonal regression and (b) PCA.

```{r, echo=FALSE}
reg$hp_z_wt_z <- Deming(train$wt_z, train$hp_z)

demi_segments <- perp_segment_coord(train$wt_z, train$hp_z,
                                    reg$hp_z_wt_z["Intercept"],
                                    reg$hp_z_wt_z["Slope"]) %>% 
  as.data.frame()

train <- train %>% mutate(
  ortho_z_1 = sqrt((demi_segments$x1)^2 +
                  (demi_segments$y1 -
                     reg$hp_z_wt_z["Intercept"])^2),
  ortho_z_2 = sqrt(
    (demi_segments$x1 - demi_segments$x0)^2 +
      (demi_segments$y1 - demi_segments$y0)^2))

train <- train %>% mutate(ortho_z_1 = case_when(
    demi_segments$x1 < 0 ~ -ortho_z_1,
    demi_segments$x1 >= 0 ~ ortho_z_1
  )) %>% 
    mutate(ortho_z_2 = case_when(
      demi_segments$y0 < demi_segments$y1 ~ -ortho_z_2,
      demi_segments$y0 >= demi_segments$y1 ~ ortho_z_2
    ))
...
reg$PC1_z_ortho_z_1 <- lm_model %>% 
  fit(PC1_z ~ ortho_z_1, data=train)
```

#### Projections

```{r, echo=FALSE, fig.width=10, fig.height=4}
plt$ortho_z_2_ortho_z_1 <- train %>% 
  ggplot(aes(x=ortho_z_1, y=ortho_z_2)) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(title="Orthogonal Regression Projection",
       x="ortho_1 of standardized variables",
       y="ortho_2 of standardized variables")

plt$PC2_z_PC1_z <- train %>% 
  ggplot(aes(x=PC1_z, y=PC2_z)) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(title="Principal Component Projection",
       x="PC1 of standardized variables",
       y="PC2 of standardized variables")

grid.arrange(plt$ortho_z_2_ortho_z_1,
             plt$PC2_z_PC1_z,
             ncol=2)
```
...
plt$PC1_z_ortho_z_1 <- train %>%
  ggplot(aes(x=ortho_z_1, y=PC1_z)) +
  geom_abline(slope=get_coef(reg$PC1_z_ortho_z_1, "ortho_z_1"),
              intercept=get_coef(reg$PC1_z_ortho_z_1),
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(title="PC1 and Orthogonal Projection of Standardized Variables",
       x="ortho_1 of standardized variables",
       y="PC1 of standardized variables",
       subtitle=str_glue("r square = {get_rsq(reg$PC1_z_ortho_z_1,3)}"))
...
* Principal components always pass through the origin, which limits their fit to uncentered data
* We can center data by converting to $z$-score, which allows PCA to optimally fit the data
* PCA is identical to projection to an orthogonal regression, once data have been standardized
...
Some variables may be correlated, so there is a *redundancy* in their information. This makes itself apparent in that if you know the value of one correlated variable, you can predict the value of the other variable. Converting these correlated variable into a single new variable seems like a good choice for reducing the number of variables while retaining information.

### Projection to Linear Regression
...
* When variables have different magnitudes, the larger magnitude variable(s) dominate the new axis.
* When variables are uncentered, PCA is completely insensitive to slope, and is only defined by the centroid.
...
* Eigen vectors: which indicate the direction(s) of maximum variance
* Eigen values: which indicate the amount of total variance represented by the corresponding eigen vector
...
### Back to The Predictive Model

We started this journey by trying to predict vehicle fuel efficiency from weight and power. After doing some linear regressions the variables independently, and then with multilinear regression, we decided to try dimension reduction to capture the effect of both weight and power in a single variabel.

So to wrap it up, let's take look at predicting fuel efficiency based on the first principal component

```{r}
reg$mpg_PC1_z <- lm_model %>% 
  fit(mpg ~ PC1_z, data=train)

plt$mpg_PC1_z <- train %>% 
  ggplot(aes(x=PC1_z, y=mpg)) +
  geom_abline(slope=get_coef(reg$mpg_PC1_z, "PC1_z"),
              intercept=get_coef(reg$mpg_PC1_z),
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(x="First Principal Component (PC1)",
       y="Fuel Efficiency (mpg)",
       title="Fuel Efficiency vs First Principal Component",
       subtitle=str_glue("r-square = {get_rsq(reg$mpg_PC1_z)}"))
