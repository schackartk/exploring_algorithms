...
segments <- perp_segment_coord(df_fake$x, df_fake$y, fake_int, fake_slope)
...
perp_segment <- perp_segment_coord(train$wt, train$hp, dr_int, dr_slope)
perp_segment <- as.data.frame(perp_segment)

reg_axis <- train %>% ggplot(aes(x=wt, y=hp)) +
  geom_segment(data=perp_segment, 
               aes(x=x0, y=y0, xend=x1, yend=y1), 
               colour=get_hurwitz_colors("viking")) +
  geom_abline(slope=dr_slope, intercept = dr_int,
              color=get_hurwitz_colors("light_gray"),
...
resc_model <- lm_model %>% 
  fit(hp ~ wt, data=rescaled)

resc_int <- summary(resc_model$fit)$coefficients["(Intercept)",1]
resc_slope <- summary(resc_model$fit)$coefficients["wt",1]

resc_segments <- perp_segment_coord(rescaled$wt, rescaled$hp,
                                    resc_int, resc_slope)
resc_segments <- as.data.frame(resc_segments)

resc_plot <- rescaled %>% ggplot(aes(x=wt, y=hp)) +
  geom_segment(data=resc_segments, 
               aes(x=x0, y=y0, xend=x1, yend=y1), 
               colour=get_hurwitz_colors("viking")) +
  geom_abline(slope=resc_slope, intercept = resc_int,
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(x="Weight (lb)",
...
norm_int <- summary(norm_model$fit)$coefficients["(Intercept)",1]
norm_slope <- summary(norm_model$fit)$coefficients["wt",1]

norm_segments <- perp_segment_coord(norm$wt, norm$hp,
                                    norm_int, norm_slope)
norm_segments <- as.data.frame(norm_segments)

norm_plot <- norm %>% ggplot(aes(x=wt, y=hp)) +
  geom_segment(data=norm_segments, 
               aes(x=x0, y=y0, xend=x1, yend=y1), 
               colour=get_hurwitz_colors("viking")) +
  geom_abline(slope=norm_slope, intercept = norm_int,
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(x="Rescaled Weight",
       ...
       subtitle="Each variable has been divided by its standard deviation") +
