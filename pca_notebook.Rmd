...
\[total\ variance\ = \sum_{n=1}^{N} \lambda_i\]
...
eig_plot <- norm_eig %>% ggplot(aes(x=eig1, y=eig2)) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(title="Eigen Projected Data")

grid.arrange(eig_plot, demi_rot_plot + labs(subtitle=element_blank()), ncol=2)
``` 

#### Correlation

```{r eig_demi_corr, echo=FALSE, warning=FALSE}
norm_eig$ortho1 = demi_rot_df$x

demi_eig_reg <- lm_model %>% 
  fit(eig1 ~ ortho1, data=norm_eig)

demi_eig_reg_int <- summary(demi_eig_reg$fit)$coefficients["(Intercept)",1]
demi_eig_reg_slope <- summary(demi_eig_reg$fit)$coefficients["ortho1",1]

demi_eig_reg_rs <- round(summary(demi_eig_reg$fit)$r.squared, digits = 3)

norm_eig %>% ggplot(aes(x=demi_rot_df$x, eig1)) +
  geom_abline(slope=demi_eig_reg_slope,
              intercept=demi_eig_reg_int,
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(x = "ortho1",
       ...
       subtitle=str_glue("r squared = {demi_eig_reg_rs}"))
```
...
norm_eig$PC1 <- pca_df$PC1

pca_eig_reg <- lm_model %>% 
  fit(PC1 ~ eig1, data=norm_eig)

pca_eig_reg_int <- summary(pca_eig_reg$fit)$coefficients["(Intercept)",1]
pca_eig_reg_slope <- summary(pca_eig_reg$fit)$coefficients["eig1",1]

pca_eig_reg_rs <- round(summary(pca_eig_reg$fit)$r.squared, digits = 3)

pca_plot <- pca_df %>% ggplot(aes(x=PC1, y=PC2)) +
  geom_point(color=get_hurwitz_colors("red")) +
  labs(title="PCA Projected Data")

grid.arrange(pca_plot, eig_plot, ncol=2)
```

#### Correlation

```{r, echo=FALSE}
pca_eig_plot <- norm_eig %>% ggplot(aes(x=eig1)) +
  geom_abline(slope=pca_eig_reg_slope,
              intercept=pca_eig_reg_int,
              color=get_hurwitz_colors("light_gray"),
              size=1) +
  geom_point(aes(y=pca_fit$x[,1]),
    color=get_hurwitz_colors("red")) +
  labs(x="eig1",
       y="PC1",
       title="PCA vs Eigenvector Projected Variables",
       subtitle=str_glue("r square = {pca_eig_reg_rs}"))
