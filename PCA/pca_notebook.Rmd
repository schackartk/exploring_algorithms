...
be looking at 3 variables: weight (`wt`), horsepower (`hp`), and fuel efficiency `mpg`.  Here is what they look like.

```{r data_select, echo=FALSE}
df <- mtcars %>% select(mpg, wt, hp)

set.seed(3456)
trainIndex <- createDataPartition(df$mpg,
                                  p = .8,
                                  list = FALSE,
                                  times = 1)
train <- df[trainIndex, ]
...
Another way to put it is that if we know the weight, we know *something* about the power, and vice-versa. This can be referred to as *redundancy* (*i.e.* similar information is represented by multiple variables). Of course the relationship isn't perfect -- we cannot predict power with perfect accuracy based on weight.
...
segments <- perp_segment_coord(df_fake$x,
                               df_fake$y,
                               get_coef(reg$ideal),
...
plt$hp_norm_wt_norm <-
  train %>% plot_perp(x = "wt_norm", y = "hp_norm") +
  labs(
    x = "Rescaled Weight",
    y = "Rescaled Power",
...
plt$mpg_reg1_fit <- train %>% ggplot(aes(x = reg1, y = mpg)) +
  geom_abline(
    slope = get_coef(reg$mpg_reg1, "reg1"),
...
plt$hp_norm_wt_norm_reg <- plot +
  geom_abline(
    slope = get_coef(reg$hp_norm_wt_norm, "wt_norm"),
...
plt$reg2_reg1_fit <- train %>% ggplot(aes(x = reg1, y = reg2)) +
  geom_abline(
    slope = get_coef(reg$reg2_reg1, "reg1"),
...
train <- train %>% add_column(ortho1 = sqrt((demi_segments$x1) ^ 2 +
...
train <- train %>% mutate(ortho1 = case_when(demi_segments$x1 < 0 ~ -ortho1,
...
plt$ortho2_orth1 <- train %>% ggplot(aes(x = ortho1, y = ortho2)) +
  geom_abline(
    slope = get_coef(reg$ortho2_ortho1, "ortho1"),
...
plt$mpg_ortho1 <- train %>% ggplot(aes(x = ortho1, y = mpg)) +
  geom_abline(
    slope = get_coef(reg$mpg_ortho1, "ortho1"),
...
grid.arrange(plt$mpg_reg1_fit + labs(x = "reg1"), plt$mpg_ortho1, ncol =
               2)
```
...
norm_eig <-
  t(eig_df$vectors) %*% t(select(train, wt_norm, hp_norm)) %>%
  t() %>% as_tibble()

train <- train %>% add_column("eig1" = norm_eig$V1,
                              "eig2" = norm_eig$V2)
...
plt$eig1_ortho1 <- train %>% ggplot(aes(x = ortho1, y = eig1)) +
  geom_abline(
    slope = get_coef(reg$eig1_ortho1, "ortho1"),
  ...
  geom_point(color = get_hurwitz_colors("red")) +
  labs(
    title = "Eigenvector Projected vs
...
train <- train %>% add_column(PC1 = pca_fit$x[, 1],
                              PC2 = pca_fit$x[, 2])
...
plt$PC1_eig1 <- train %>% ggplot(aes(x = eig1, y = PC1)) +
  geom_abline(
    slope = get_coef(reg$PC1_eig1, "eig1"),
...
```{r, echo = false}
reg$dem_zero_int <- c(slope = mean(train$hp_norm) / mean(train$wt_norm),
                      intercept = 0)
...
```{r, echo=FALSE}
ortho_no_segments <- perp_segment_coord(train$wt_norm,
                                        train$hp_norm,
                                        reg$dem_zero_int["intercept"],
                                        reg$dem_zero_int["slope"]) %>%
  as.data.frame()

train <- train %>% add_column(ortho_no_1 = sqrt((ortho_no_segments$x1) ^
                                                  2 +
                                                  (ortho_no_segments$y1 - reg$dem_zero_int["intercept"]) ^
                                                  2
),
ortho_no_2 = sqrt(
...
train <- train %>% mutate(
  ortho_no_1 = case_when(
    ortho_no_segments$x1 < 0 ~ -ortho_no_1,
    ortho_no_segments$x1 >= 0 ~ ortho_no_1
  )
) %>%
  mutate(
    ortho_no_2 = case_when(
...
train %>% ggplot(aes(x = ortho_no_1, y = PC1)) +
  geom_abline(
    slope = get_coef(reg$PC1_ortho_no_1, "ortho_no_1"),
...
plt$y_2_x_2 <- df_fake %>%
  ...
  geom_abline(
    slope = mean(df_fake$y_2) / mean(df_fake$x_2),
    intercept = c(0, 0),
    color = get_hurwitz_colors("light_gray"),
    size = 1
  ) +
...
train <- train %>% mutate(ortho_z_1 = sqrt((demi_segments$x1) ^ 2 +
                                             (demi_segments$y1 -
...
train <- train %>% add_column(PC1_z = pca_z_fit$x[, 1],
                              PC2_z = pca_z_fit$x[, 2])
...
plt$PC1_z_ortho_z_1 <- train %>%
  ...
  geom_abline(
    slope = get_coef(reg$PC1_z_ortho_z_1, "ortho_z_1"),
  ...
  geom_point(color = get_hurwitz_colors("red")) +
  labs(
    title = "PC1 and Orthogonal Projection of Standardized Variables",
    x = "ortho_1 of standardized variables",
...
plt$mpg_PC1_z <- train %>%
  ...
  geom_abline(
    slope = get_coef(reg$mpg_PC1_z, "PC1_z"),
